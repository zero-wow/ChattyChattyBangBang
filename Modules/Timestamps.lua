local mod = ChattyChattyBangBang:NewModule("Timestamps", "AceHook-3.0","AceEvent-3.0")
...
function mod:OnInitialize()
	self.db = ChattyChattyBangBang.db:RegisterNamespace("Timestamps", defaults)
end
...
function mod:AddMessage(frame, text, ...)
	...
	if id and self.db.profile.frames["Frame"..id] and not(CHAT_TIMESTAMP_FORMAT) then
		if not ChattyChattyBangBang.loading then
			if not text then 
