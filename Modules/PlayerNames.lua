local mod = ChattyChattyBangBang:NewModule("Player Class Colors", "AceHook-3.0", "AceEvent-3.0")
...
do
	...
	local function HSVtoRGB(h, s, v)
	   ...
	   elseif i == 5 then
		  ...
		  DEFAULT_CHAT_FRAME:AddMessage("ChattyChattyBangBang HSVtoRGB failed")
	   end
	   if not t.r then t.r = 0 end
	   if not t.g then t.g = 0 end
	   if not t.b then t.b = 0 end
...
function mod:OnInitialize()
	...
	self.db = ChattyChattyBangBang.db:RegisterNamespace("PlayerNames", defaults)
...
function mod:OnEnable()
	...
	if self.db.profile.useTabComplete then
		AceTab:RegisterTabCompletion("ChattyChattyBangBang", nil, tabComplete)
	end
...
function mod:OnDisable()
	if AceTab:IsTabCompletionRegistered("ChattyChattyBangBang") then
		AceTab:UnregisterTabCompletion("ChattyChattyBangBang")
	end
		...
		options = {
			...
			useTabComplete = {
				...
				set = function(info, v)
					...
					if v and not AceTab:IsTabCompletionRegistered("ChattyChattyBangBang") then
						AceTab:RegisterTabCompletion("ChattyChattyBangBang", nil, tabComplete)
					elseif not v and AceTab:IsTabCompletionRegistered("ChattyChattyBangBang") then
						AceTab:UnregisterTabCompletion("ChattyChattyBangBang")
					end
				end
			},
