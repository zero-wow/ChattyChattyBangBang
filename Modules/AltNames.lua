local mod = ChattyChattyBangBang:NewModule("Alt Linking", "AceHook-3.0", "AceEvent-3.0")
...
function mod:OnInitialize()
	self.db = ChattyChattyBangBang.db:RegisterNamespace("AltLinks", defaults)
end
...
function mod:OnEnable()
	...
	self.colorMod = ChattyChattyBangBang:GetModule("Player Class Colors")
end
...
function mod:GUILD_ROSTER_UPDATE(event,arg1)
	...
	if arg1 or (GuildFrame and GuildFrame.IsVisible and GuildFrame:IsVisible()) or doscan then
		doscan=false
		mod:ScanGuildNotes()
	end
...
function mod:ScanGuildNotes()
	...
	for i=1,GetNumGuildMembers(true) do
		...
		-- Ascension can briefly return a roster row before its note/rank fields
		-- are populated.  The original Chatter code passed those nil values to
		-- strlower, which is a hard Lua error when native fallback is active.
