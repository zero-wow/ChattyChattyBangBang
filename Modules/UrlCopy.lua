local mod = ChattyChattyBangBang:NewModule("URL Copy", "AceHook-3.0")
...
do
	...
	function mod:OnInitialize()
		self.db = ChattyChattyBangBang.db:RegisterNamespace("UrlCopy", defaults)
	end	
...
StaticPopupDialogs["ChattyChattyBangBangUrlCopyDialog"] = {
...
function mod:SetItemRef(link, text, button)
	if sub(link, 1, 3) == "url" then
		...
		StaticPopup_Show("ChattyChattyBangBangUrlCopyDialog")
		return
	end
