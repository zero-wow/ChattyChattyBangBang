local mod = ChattyChattyBangBang:NewModule("Scrollback")
...
local function setlines(frame, lines)
	if frame:GetMaxLines() ~= lines then 
		...
		ChattyChattyBangBang.loading = true

		for k,v in pairs(history) do
			frame:AddMessage(unpack(v))
		end

		ChattyChattyBangBang.loading = false
...
function mod:OnInitialize()
	self.db = ChattyChattyBangBang.db:RegisterNamespace("Scrollback", defaults)
